...
renamed as (
    select
        ...
        {{ safe_cast('rate_code_id', 'integer') }} as rate_code_id,
        cast(pick_up_location_id as integer) as pickup_location_id,
        cast(drop_off_location_id as integer) as dropoff_location_id,

        -- timestamps
        cast(pick_up_datetime as timestamp) as pickup_datetime,  -- lpep = Licensed Passenger Enhancement Program (green taxis)
        cast(drop_off_datetime as timestamp) as dropoff_datetime,

        -- trip info
        cast(store_and_forward_flag as string) as store_and_fwd_flag,
